...
pub mod blueshift_anchor_flash_loan {
    ...
    pub fn borrow(ctx: Context<Loan>, borrow_amount: u64) -> Result<()> {
        ...
        let ixn = ctx.accounts.instructions.to_account_info();

        let current_index = load_current_index_checked(&ixn)?;

        require_eq!(current_index, 0, ProtocolError::InvalidInstructionIndex);

        let data = ixn.try_borrow_data()?;

        let len = u16::from_le_bytes(data[0..2].try_into().unwrap()) as usize;

        match load_instruction_at_checked(
            ...
            Err(_) => return Err(ProtocolError::MissingRepayIx.into()),
        }
    ...
    pub fn repay(ctx: Context<Loan>) -> Result<()> {
        ...
        match load_instruction_at_checked(0, &ixs) {
            ...
            Err(_) => return Err(ProtocolError::MissingBorrowIx.into()),
        }
